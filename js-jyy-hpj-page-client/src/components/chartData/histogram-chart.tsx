...
const HistogramChart = forwardRef((props: HistogramProps, ref): JSX.Element => {
    ...
    chart.current.setOption({
      ...
      legend: {
        data: data?.map((item) => {
          ...
          // if (type === 'norm') {
          //   const tu = indicator && `${indicator} - ${database} - ${type}`;
          //   return tu;
          // }
          return indicator;
        }),
      ...
      series: data?.map((d) => {
        const { indicator, resultList } = d || {};
        return {
          name: indicator,
          data: resultList?.map((dv) => dv?.value),
          type: 'bar',
          barGap: 0,
          barMaxWidth: 20,
        };
